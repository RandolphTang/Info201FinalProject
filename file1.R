...
summarize_df <- summarize(group_df, mean.Start.Mid.Salary.Difference = mean(Start.Mid.Salary.Diff))

max_startMedianSalary_certain_type <- function(df, collegeType){
  certain_type_startMedianSalary <- filter(df, df$School.Type == collegeType, na.rm = TRUE)
  
  max_value <- max(certain_type_startMedianSalary$Starting.Median.Salary.x, na.rm = TRUE)
  
  return(max_value)
}

min_startMedianSalary_certain_type <- function(df, collegeType){
  certain_type_startMedianSalary <- filter(df, df$School.Type == collegeType, na.rm = TRUE)
  
  min_value <- min(certain_type_startMedianSalary$Starting.Median.Salary.x, na.rm = TRUE)
